...
def main(base):
    ...
    th = d["meta"].get("throughline")
    if not th:
        E.append("meta.throughline 없음 — 상단 배지에 둘 관통 단어가 필요합니다 "
                 "(기준: 공단에 전화해서 말해야 하는 단어)")
    else:
        # 관통 단어는 모든 화면에 남는 유일한 주제어입니다.
        # 쇼츠 썸네일은 유튜브가 아무 프레임이나 고르고 나중에 바꿀 수도 없습니다.
        # 어느 프레임이 걸려도 이 단어 하나로 무슨 제도인지 알 수 있어야 합니다.
        #
        # 실제로 근로장려금 편이 '반기신청'으로 나갔습니다. 7개 프레임 중 6개에
        # '근로장려금'이 한 글자도 없었고, 유튜브가 하필 그중 하나를 썸네일로 집었습니다.
        # 채널 목록에서 그 영상이 무슨 영상인지 알 수 없었습니다.
        PROC = ("신청", "조회", "접수", "발급", "납부", "제출", "등록", "변경", "취소", "정산")
        hit = [w for w in PROC if th.endswith(w)]
        if hit:
            E.append(f"관통 단어 '{th}' 는 절차 이름입니다('{hit[0]}'으로 끝남) — "
                     f"제도 이름을 쓰세요. 이 단어는 모든 프레임에 남는 유일한 주제어이고, "
                     f"쇼츠 썸네일은 유튜브가 아무 프레임이나 집습니다")
        elif len(th) < 3:
            W.append(f"관통 단어 '{th}' 가 너무 짧습니다 — 그 단어만 보고 제도를 알 수 있어야 합니다")
        else:
            ok(f"관통 단어 '{th}' — 모든 프레임에 남습니다")
    ...
    if blog.exists():
        ...
        # --- 3c. 표를 위치로 가리키지 않습니다 --------------------------
        # '첫 열' · '왼쪽 열' 은 표가 재배치되면(모바일 · 카드 · 영상) 뜻이 어긋납니다.
        # 독자도 열을 세어야 합니다. 헤더에 적힌 이름으로 가리킵니다.
        POS = ["첫 열", "마지막 열", "왼쪽 열", "오른쪽 열", "첫 번째 열",
               "두 번째 열", "세 번째 열", "왼쪽 칸", "오른쪽 칸", "윗 줄", "아랫 줄"]
        for i, ln in enumerate(txt.split("\n"), 1):
            hit = [w for w in POS if w in ln]
            if hit:
                E.append(f"L{i}: 표를 위치로 가리킴 {hit} — 표가 재배치되면 어긋납니다. "
                         f"헤더에 적힌 이름으로 쓰세요")
    ...
    # 썸네일 파일 — 쇼츠 썸네일은 올린 뒤에도 스튜디오(웹)에서 바꿀 수 있습니다.
    # 손으로 올려야 하므로 파일이 없으면 그 단계가 통째로 빠집니다.
    if vid.exists() and not (base/"thumb.png").exists():
        W.append("thumb.png 없음 — 다시 렌더하면 0.00초 프레임이 저장됩니다")
